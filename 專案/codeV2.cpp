...
bool doBestCombination(Store *retailer, Lc *center, double **canEarn);
...
int main()
{
    ...
    while (true)
    {
        bool det = doBestCombination(retailer, center, canEarn);
        if (det == false)
            break;
    }
    // 開始輸出
...
{
    bool have = false;
    double **tempCanEarn = new double *[n];
    for (int i = 0; i < n; i++)
    {
        tempCanEarn[i] = new double[m];
    }
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            tempCanEarn[i][j] = canEarn[i][j];
        }
    }
    int bestLc = 0, bestSto = 0, max = INT_MIN;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (canEarn[i][j] > max && canEarn[i][j] > 0)
            {
                bestLc = i;
                bestSto = j;
                max = canEarn[i][j];
    ...
    if (have)
    {
        int capacity = center[bestLc].capacity;
        // 算可不可以賺錢
        for (int i = 0; i < m; i++)
        {
            int betterSto = 0;
            for (int j = 0; j < m; j++)
            {
                if (tempCanEarn[bestLc][betterSto] < tempCanEarn[bestLc][j])
                {
                    betterSto = j;
                }
            }
            if (tempCanEarn[bestLc][betterSto] <= 0)
            {
                break;
            }
            int deliveryNum = (capacity >= retailer[betterSto].demand ? retailer[betterSto].demand : capacity);
            center[bestLc].buildingCost -= findRevenue(retailer[betterSto], center[bestLc], deliveryNum);
            capacity -= deliveryNum;
            tempCanEarn[bestLc][betterSto] = 0;
            if (capacity == 0)
            {
                break;
            }
        }
        if (center[bestLc].buildingCost < 0)
        {
            for (int i = 0; i < m; i++)
            {
                int betterSto = 0;
                for (int j = 0; j < m; j++)
                {
                    if (canEarn[bestLc][betterSto] < canEarn[bestLc][j])
                    {
                        betterSto = j;
                    }
                }
                if (canEarn[bestLc][betterSto] <= 0)
                {
                    break;
                }
                canEarn[bestLc][betterSto] = 0;
                int deliveryNum = (center[bestLc].capacity >= retailer[betterSto].demand ? retailer[betterSto].demand : center[bestLc].capacity);
                ...
                retailer[betterSto].fromLc[bestLc] = deliveryNum; //紀錄補貨數量
